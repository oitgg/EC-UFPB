...
export const fetchNotifcations = () => (dispatch, getState) => {
  ...
  .then(({data}) => {
    console.log('notificações capturadas com sucesso', data);
    dispatch({
    ...
    console.log('captura das notificações falhou', error);
    dispatch({
...
export const fetchNotificationCount = () => (dispatch, getState) => {
  ...
  .then(({data}) => {
    console.log('captura da contagem com sucesso', data);
    dispatch({
      type: FETCH_NOTIFICATION_COUNT,
      count: data.count,
    });
  }, (error) => {
    console.log('captura da contagem falhou', error);
  })
};
